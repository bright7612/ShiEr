...
class IssueController extends AdminController
{
...

    {
        if (IS_POST) {
            ...
            $builder->title("添加分类")->keyId()->keyText('title', L('_TITLE_'))->keySelect('pid',L('_FATHER_CLASS_'), L('_FATHER_CLASS_SELECT_'), array('0' =>L('_TOP_CLASS_')) + $opt)
    ...
    public function contents($page = 1, $r = 10)
    {
        ...
        $builder->title(L('_CONTENT_MANAGE_'))
            ...
            ->keyId()->keyLink('title', L('_TITLE_'), 'Issue/Index/issueContentDetail?id=###')->keyUid()->keyCreateTime()->keyStatus()->keyDoActionEdit("editArticle?id=###")
            ->data($list)
            ->pagination($totalCount, $r)
            ->display();
    }
   ...
   public function editArticle()
    {
        ...
        if (IS_POST) {
            ...
            $data = M("issue_content")->where(array("id" => $id))->find();
            $builder = new AdminConfigBuilder();

            $dzztypeopt = array();
			$mapcontent['status']=1;
			$mapcontent['pid']=array("gt",0);

	        $opt = array();
	        $issues = M("issue")->where($mapcontent)->field("id,title")->select();
	        foreach ($issues as $issue) {
	            $opt[$issue['id']] = $issue['title'];
	        }
    ...
    public function verify($page = 1, $r = 10)
    {
        ...
        $builder->title(L('_CONTENT_AUDIT_'))
            ->setStatusUrl(U('setIssueContentStatus'))->buttonEnable('', L('_AUDIT_SUCCESS_'))->buttonDelete()->buttonNew(U("editArticle"))
            ->keyId()->keyLink('title', L('_TITLE_'), 'Issue/Index/issueContentDetail?id=###')->keyUid()->keyCreateTime()->keyStatus()->keyDoActionEdit("editArticle?id=###")
            ->data($list)
            ->pagination($totalCount, $r)
            ->display();
